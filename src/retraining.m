% Code excerpts from https://se.mathworks.com/help/deeplearning/examples/transfer-learning-using-alexnet.html
% The above refereed code was adapted for the current classification task
...
options = trainingOptions('adam','InitialLearnRate',1e-4,'ValidationData',augimdsVal,'ValidationFrequency',20,'LearnRateSchedule','piecewise','LearnRateDropPeriod',1,'LearnRateDropFactor',0.6,'MaxEpochs', 4, 'MiniBatchSize', 128, 'Plots','training-progress' ); 
% Experiementally determined parameters. Refer Results

netTransfer = trainNetwork(augimdsTrain,layers,options);  

%% Performance Evaluation (To avoid Retraining uncomment the lines 107-110 and )
%load('trainedWt.mat') % Download from: https://drive.google.com/open?id=1giYkHDNWAeZox2xCHJ7R8TABSIaruT5w
%imds_test = imageDatastore('test');
%inputSize = netTransfer.Layers(1).InputSize
%augimdsTest = augmentedImageDatastore(inputSize(1:2),imds_test); % Resizing to CNN size

[YPred,scores] = classify(netTransfer,augimdsTest) % Accuracy computation
% Make sure to execute Lines 41-65 to get the labels before the next line.
